...
/**
 * a game canvas which runs an animation of pictures which explain the player his time has ran out
 * so the game is over and he lost.
 *
 * @author class3
 */
public class TimeOutCanvas extends GameCanvas implements Runnable {
 ...
 */
    ...
    public void init() {
        ...
        g = getGraphics();
        try {
            mp = new MyMediaPlayer("/Sound/outOfTime.mp3");
            mp.setVolume(100);
            mp.setLoopCount(1);
            mp.play();
        ...
        while (i <= 8) {
            try {
                ...
                img = Image.createImage("/Pics/timesOut" + imgIndex + ".png");
        ...
        try {
            img = Image.createImage("/Pics/timesOut1.png");
        } catch (IOException ex) {
            ex.printStackTrace();
        }
        g.drawImage(img, 0, 0, Graphics.TOP | Graphics.LEFT);
