...
/**
 * a game canvas which runs an animation of pictures which says the worker has
 * died
 *
 * @author class3
 */
public class DeathCanvas extends GameCanvas implements Runnable {
    ...
    public void init() {
        ...
        g = getGraphics();
        try {
            mp = new MyMediaPlayer("/Sound/dead.mp3");
            mp.setVolume(100);
            mp.setLoopCount(1);
            mp.play();
        ...
        while (i <= 6) {
            try {
                ...
                img = Image.createImage("/Pics/death" + imgIndex + ".png");
